...
void odom_estimation(){
    while(1){
        if(!pointCloudEdgeBuf.empty() && !pointCloudSurfBuf.empty()){
            ...
            br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), map_tf_name, baselink_tf_name));

            // publish odometry
            nav_msgs::Odometry laserOdometry;
            laserOdometry.header.frame_id = map_tf_name;
            laserOdometry.child_frame_id = baselink_tf_name;
...
int main(int argc, char **argv)
{
    ...
    pubLaserOdometry = nh.advertise<nav_msgs::Odometry>(odom_topic_name, 100);
